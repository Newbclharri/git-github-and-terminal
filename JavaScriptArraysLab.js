...
foods.splice(1,1,"sushi","cupcake") //delete pizza at index 1 and add 'sushi' and 'cupcake'
...
// const fromFoods = foods.slice(1,3)
...
const hasSoup = foods.indexOf("soup") === -1 ? false : true
...
let num = getNumber(numArrays);

function getNumber(array){
    const arrays = array

    let item = null;
    for(list = 0; list < arrays.length; list++){
        for(index = 0; index < arrays[list].length; index++){
            if(arrays[list][index] / 11 === 6){
                item = arrays[list][index]
                break;
            }
        }
        if(item / 11 === 6){
